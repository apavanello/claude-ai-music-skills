...
Source verification is tracked per-track in each track file's `Sources Verified` field (single source of truth). Use the `resume` skill or the `validate-album` skill to see verification status across all tracks.
...
### Image Prompt
*Generated by the `album-art-director` skill Platform-specific format.*

```
[Prompt will be generated in the format matching your selected platform.
Run the `album-art-director` skill to create a visual concept and prompt.]
```
...
Save generated album art using the `import-art` skill or manually to these locations:
